    ...
    <script>
        ...
        function cargarMedioPago() {
            ...
            let medioPago = '<div style="width: 100%;" id="medioPago' + consMedio +
                ...
                '           " onchange="cambioFormatoMedioPago(this.id);" onkeypress="return validartxtnum(event)" onclick="this.select();" class="form-control text-end" id="valorVisPago' +
                ...
                '     </div>' +
                ' </div>' +
                ' <div style="display: none;" class="col-xl-1 col-md-3 col-3 align-content-end">' +
        ...
        function cambioFormatoMedioPago(id) {
            let numero = document.getElementById(id)
            let valorVenta = document.getElementById("valotTotalVentPaq").value
            ...
            if (parseFloat(numero.value) > parseFloat(valorVenta)) {
                ...
                document.getElementById("valorVisPago1").value = "$ 0,00"
                document.getElementById("valorPago1").value = "0"
                return
            }

            // validar si el valor a pagar e menor al total de la venta se debe seleccionar como un abono
            if (parseFloat(numero.value) < parseFloat(valorVenta)) {
                swal("¡Atención!", "Si intenta hacer un pago menor al valor de la venta, debe ser ingresado como un abono.",
                    "warning")
                document.getElementById("valorVisPago1").value = "$ 0,00"
                document.getElementById("valorPago1").value = "0"
                return
            ...
            document.getElementById("valorPago1").value = numero.value
            let formatoMoneda = formatCurrency(numero.value, 'es-CO', 'COP')
            numero.value = formatoMoneda
        ...
        function verPago(idVenta) {
            ...
            fetch(url, {
                ...
                .then(response => response.json())
                .then(data => {
                    document.getElementById("descripcionPaquete").innerText = data.historialpagos.descripion_paquete
                        .descripcion
                    document.getElementById("sesionesPaquete").innerText =
                        `${data.historialpagos.sesiones_disponibles} sesión disponible de ${data.historialpagos.sesiones_compradas}`
                    document.getElementById("valorPaquete").innerText = formatCurrency(data.historialpagos.monto_total,
                        'es-CO', 'COP')
                    // pagos realizados 
                    let html = ""
                        ...
                        html += `<div class="box mb-15 pull-up">
                                    <div class="box-body ">
                                        <div class="d-flex align-items-center justify-content-between">
                                            ...
                                            <button class="btn btn-primary btn-sm" onclick="imprimirPago('${pago.fecha_pago}', '${pago.pago_realizado}', '${pago.nombre_usuario}')">
